...
} = require('./services/RAG/vector.search.service');
...
router.post('/multi-agent', async (req, res) => {
  ...
  try {
    ...
    const summaryPromises = documents.map(async (doc) => {
      ...
      const finalSummary = await processChunksSequentially(
        chunks,
        conversation,
    ...
    res.json({
      responses: [
        ...
        {
          role: 'assistant',
          content: managerResponse.data.choices[0].message.content,
        },
      ],
    });
  } catch (error) {
    res.status(500).json({
      error: 'Error interacting with OpenAI API',
...
router.post('/chatgpt-direct', async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: 'Conversation history cannot be empty' });
    }
    ...
    res
      .status(500)
      .json({ error: 'Error with ChatGPT API', details: error.message });
  }
});
...
router.post('/llama3', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Error with Groq API', details: error.message });
  }
});
...
router.post('/gemini', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Error with Gemini API', details: error.message });
  }
});
